...
class TemplateService {
  ...
  Future<String> readTemplate(String name) async {
    ...
    try {
      final packageUri =
          Uri.parse('package:finvoras_gen/src/templates/$name.txt');
    ...
    try {
      ...
      final packageRoot =
          p.dirname(p.dirname(scriptPath)); // assuming bin/script.dart -> root
      final relativePath =
          p.join(packageRoot, 'lib', 'src', 'templates', '$name.txt');
    ...
    final fallbackPath =
        p.join(Directory.current.path, 'lib', 'src', 'templates', '$name.txt');
